<?php
/**
 * foundryforged Theme Customizer.
 *
 ...
 */
function foundryforged_customize_register( $wp_customize ) {
	...
	$wp_customize->add_control(
		new WP_Customize_Color_Control(
			...
			'theme_bg_color', 
				array(
					'label'		=> __( 'Header and footer background color', 'foundryforged'),
					'section'	=> 'colors',
	...
	$wp_customize->add_control(
		new WP_Customize_Color_Control(
			...
			'interactive_color', array(
				'label'		=> __( 'Interactive color (links etc)', 'foundryforged' ),
				'section'	=> 'colors',
	...
	$wp_customize->add_section( 'theme_options', 
		array(
			'title'			=> __( 'Theme Options', 'foundryforged' ),
			'priority'		=> 95,
			'capability'	=> 'edit_theme_options',
			'description'	=> __( 'Change how much of a post is displayed on index and archive pages.', 'foundryforged' )
		)
	);
	...
	$wp_customize->add_setting(	'length_setting',
		array(
			'default'			=> 'excerpt',
			'type'				=> 'theme_mod',
			'sanitize_callback' => 'foundryforged_sanitize_length', // Sanitization function appears further down
			'transport'			=> 'postMessage'
		)
	);

	// Add the controls
	$wp_customize->add_control(	'foundryforged_length_control',
		array(
			'type'		=> 'radio',
			'label'		=> __( 'Index/archive displays', 'foundryforged' ),
			'section'	=> 'theme_options',
			'choices'	=> array(
				'excerpt'		=> __( 'Excerpt (default)', 'foundryforged' ),
				'full-content'	=> __( 'Full content', 'foundryforged' )
			),
...
add_action( 'customize_register', 'foundryforged_customize_register' );

/**
 * Binds JS handlers to make Theme Customizer preview reload changes asynchronously.
 */
function foundryforged_customize_preview_js() {
	wp_enqueue_script( 'foundryforged_customizer', get_template_directory_uri() . '/js/customizer.js', array( 'customize-preview' ), '20151215', true );
}
add_action( 'customize_preview_init', 'foundryforged_customize_preview_js' );
...
function foundryforged_sanitize_length( $value ) {
...
if ( ! function_exists( 'foundryforged_header_style' ) ) :
/**
 * Styles the header image and text displayed on the blog.
 *
 * @see foundryforged_custom_header_setup().
 */
function foundryforged_header_style() {
